...
describe("MultiSigWallet", () => {
    ...
    beforeEach(async () => {
        ...
        multiSigWallet = await contractFactory.deploy(walletName, wallets, 3, { value: initialContractBalance });
    });

    describe("Deployment", () => {
        it("Reverts if number of wallets is less than MINIMUM_WALLETS (3)", async() => {
            let smallWalletSet = [deployer.address, user.address];
            await expect(contractFactory.deploy(walletName, smallWalletSet, 2)).to.be.revertedWith(
                "NotEnoughWallets"
            );
        });
        it("Reverts if required approvals is less 50% or less than the number of wallets", async() => {
            await expect(contractFactory.deploy(walletName, wallets, 2)).to.be.revertedWith(
    ...
    describe("Proposes transaction", () => {
        ...
        it("Creates a transaction", async() => {
            ...
            let transaction = await multiSigWallet.getTransaction(0);
        ...
        it("Adds the proposer approve to the transaction", async() => {
            await multiSigWallet.proposeTransaction(deployer.address, parseEther("1"));
            expect(await multiSigWallet.hasApproved(0, deployer.address)).to.equal(true);
        })
    });

    describe("Approves transaction", () => {
        it("Reverts if not a owner", async() => {
            await expect(multiSigWallet.connect(notOwner).approveTransaction(0)).to.be.revertedWith(
                "NotOwner"
            );
        });
        it("Reverts if transaction does not exist", async() => {
            await expect(multiSigWallet.approveTransaction(0)).to.be.revertedWith(
        ...
        it("Adds the approval to the transaction", async() => {
            ...
            expect(await multiSigWallet.hasApproved(0, user.address)).to.equal(false);
            // Approve with `user`
            await multiSigWallet.connect(user).approveTransaction(0);
            // Should be true for the `user` after approving
            expect(await multiSigWallet.hasApproved(0, user.address)).to.equal(true);
        });
    });
    ...
    describe("Executes transaction", () => {
        ...
        it("Reverts if the transaction was already executed", async() => {
            ...
            await multiSigWallet.connect(user).approveTransaction(0);
            await multiSigWallet.connect(user2).approveTransaction(0);
            // Execute transaction
        ...
        it("Successfully moves money from contract to transaction destination", async() => {
            ...
            await multiSigWallet.connect(user).executeTransaction(0);
